...
نکته بسیار مهم: لطفاً ساختار زمانی فایل SRT را دقیقاً حفظ کن. یعنی هر خط ترجمه فارسی باید دقیقاً مقابل خط اصلی انگلیسی و با همان شماره و زمان‌بندی قرار گیرد. فقط متن انگلیسی را ترجمه کن و اعداد و زمان‌بندی را بدون تغییر کپی کن.
...
{target_lang_name} Translation:"""
    ...
    def _extract_persian_text(self, line: str) -> str:
        """استخراج متن فارسی از خطی که شامل هر دو زبان است"""
        try:
            # تقسیم خط به کلمات
            words = line.split()
            persian_words = []
            ...
            for word in words:
                ...
                if self._is_persian_word(word):
                    persian_words.append(word)
            
            # اگر کلمه فارسی پیدا شد، آن‌ها را برگردان
            if persian_words:
                return ' '.join(persian_words)
            
            # اگر هیچ کلمه فارسی پیدا نشد، بررسی کن که آیا کل خط فارسی است
    ...
    def _create_fixed_text_filter(self, config: dict) -> str:
        ...
        try:
            ...
            color = self._color_to_hex(config['color'])
            margin_bottom = config['margin_bottom']
            
            # استفاده از فیلتر ساده drawtext بدون فونت مشخص
            # FFmpeg خودکار فونت مناسب را انتخاب می‌کند
            filter_text = f"drawtext=text='{text}':x=(w-text_w)/2:y=h-text_h-{margin_bottom}:fontsize={fontsize}:fontcolor={color}"
    ...
    def _get_persian_font_path(self) -> str:
        ...
        try:
            ...
            # اولویت با فونت‌هایی که قطعاً از فارسی پشتیبانی می‌کنند
            persian_fonts = [
